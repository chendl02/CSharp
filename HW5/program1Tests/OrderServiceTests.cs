...
namespace program1.Tests
{
    ...
    public class OrderServiceTests
    {
        ...
        public void FindOrderByClientNameTest()
        {
            ...
            orderService.AddOrder(order1);
            orderService.AddOrder(order2);
            List<Order> list = orderService.FindOrderByClientName("chen");
            Assert.AreEqual(list.Count, 2);
            foreach (var order in list)
            {
                Assert.AreEqual(order.ClientName, "chen");
            }
        ...
        public void ExportTest()
        {
            ...
            orderService.AddOrder(order1);
            orderService.AddOrder(order2);
            orderService.Export(@".\orderService.xml");
            System.IO.FileInfo file = new System.IO.FileInfo(@".\orderService.xml");
        ...
        {
            OrderService orderService = OrderService.Import(@".\orderService.xml");
            Assert.AreEqual(orderService.Orders[0].ClientName, "chen");
        }
    }
}